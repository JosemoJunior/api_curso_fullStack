...
public class FornecedorService {
    ...
    public void criarFornecedor (Fornecedor fornecedor) {
        ...
        //antes de salvar no banco de dados, preciso anexar o endereco no fornecedor
    ...
    public Optional<Fornecedor> buscarFornecedorPorIr(Long id) {
    ...
    public void atualizarFornecedorPorId(Long id, Fornecedor fornecedor) {
        // PRIMEIRO PASSO: VER SE O FORNECEDOR EXISTE, PPARA DEPOIS SOBRESCREVER NELE.
        Optional<Fornecedor> fornecedorDoBancodeDados = buscarFornecedorPorIr(id);
        // SE NÃO EXISTIR
        if(fornecedorDoBancodeDados.isEmpty()) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Fornecedor não encontrado!");
        }
        // SE CHEGAR AQUI, SIGNIFICA QUE EXISTE FORNECEDOR COM ESSE ID, ENTÃO VAMOS ATUALIZAR
        Fornecedor fornecedorSobrescrito = fornecedorDoBancodeDados.get();

        // COM ESSE FORNECEDOR QUE VAI SER SOBRESCRITO ACIMA, FAÇO OS SETS NECESSÄRIOS PARA ATUALIZAR
